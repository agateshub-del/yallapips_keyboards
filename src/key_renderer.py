...
def render_long_display(profit: float = 0.0,
    ...
    """
    90x432 portrait — fills the full long strip display (device key 18).
    Hardware flips vertically; set_long_display pre-flips to compensate.
    Three equal sections of 144px each: SESSION / LOCAL TIME / OPEN P&L
    """
    W, H = 90, 432
    ...
    def fit(text, max_sz, bold=False):
    ...
    # ── SESSION (y 0..143) ────────────────────────────────────────
    draw.rectangle([(0, 0),   (W-1, SH-1)], fill=tuple(c//6 for c in sc))
    draw.rectangle([(0, 0),   (W-1, 6)],    fill=sc)
    draw.ellipse(  [(PAD, 12),(PAD+12, 24)], fill=sc)
    draw.text((CX, 40),  "SESSION",
              font=_font(11), fill=(80,90,110), anchor="mm")
    draw.text((CX, 82),  sn,
              font=fit(sn, 21, True), fill=sc, anchor="mm")
    draw.text((CX, 116), utc_now.strftime("%H:%M"),
              font=_font(24, True), fill=sc, anchor="mm")
    draw.text((CX, 136), "UTC",
              font=_font(10), fill=(70,80,100), anchor="mm")
    draw.rectangle([(0, SH-1),(W-1, SH)], fill=(25,30,45))

    # ── LOCAL TIME (y 145..288) ───────────────────────────────────
    y2 = SH + 1
    draw.rectangle([(0, y2),(W-1, y2+SH-1)], fill=(10,12,20))
    draw.text((CX, y2+22),  "LOCAL TIME",
              font=_font(11), fill=(55,65,85), anchor="mm")
    draw.text((CX, y2+70),  local_now.strftime("%H:%M"),
              font=_font(30, True), fill=(215,225,245), anchor="mm")
    draw.text((CX, y2+100), local_now.strftime(":%S"),
              font=_font(22, True), fill=(150,165,190), anchor="mm")
    draw.text((CX, y2+122), local_now.strftime("%d %b"),
              font=_font(15, True), fill=(85,95,115), anchor="mm")
    draw.text((CX, y2+140), local_now.strftime("%Y"),
              font=_font(11), fill=(60,70,90), anchor="mm")
    ...
    if connected:
        ...
        bg3 = tuple(c//6 for c in pc)
        draw.rectangle([(0, y3),  (W-1, H-1)],  fill=bg3)
        draw.rectangle([(0, H-7), (W-1, H-1)],  fill=pc)
        draw.text((CX, y3+22),  "OPEN P&L",
                  font=_font(11), fill=(60,70,60), anchor="mm")
        draw.text((CX, y3+70),  "▲" if profit >= 0 else "▼",
                  font=_font(34, True), fill=pc, anchor="mm")
        draw.text((CX, y3+112), f"{profit:+.2f}",
                  font=fit(f"{profit:+.2f}", 24, True), fill=pc, anchor="mm")
        draw.text((CX, y3+136), currency,
                  font=_font(13), fill=(60,70,60), anchor="mm")
    else:
        draw.rectangle([(0, y3),(W-1, H-1)],  fill=(14,8,8))
        draw.rectangle([(0, H-7),(W-1, H-1)], fill=(80,30,30))
        draw.text((CX, y3+60), "MT5",     font=_font(26,True), fill=(140,40,40), anchor="mm")
        draw.text((CX, y3+95), "OFFLINE", font=_font(16),      fill=(110,30,30), anchor="mm")

    draw.rectangle([(0,0),(W-1,H-1)], outline=(28,34,50), width=1)
    return img
