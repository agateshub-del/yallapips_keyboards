...
def show_about():
    ...
    root = tk.Tk()
    root.title("About YALLA PIPS Keyboard")
    ...
    tk.Label(root, text="XAUUSD Smart Money Concepts", bg="#0a0c14", fg="#555",
             font=("Arial",9)).pack()
    tk.Frame(root, bg="#ffd600", height=1, width=240).pack(pady=12)
...
def show_settings_window():
    ...
    def on_save():
        updates = {
            ...
            "mt5_bridge_host": bridge_host_var.get().strip(),
            "mt5_bridge_port": int(bridge_port_var.get() or 9999),
        }
        errors = []
...
def start_tray(on_quit):
    icon = pystray.Icon(
        ...
        menu=pystray.Menu(
            ...
            pystray.MenuItem("About YALLA PIPS",   lambda: threading.Thread(target=show_about,daemon=True).start()),
