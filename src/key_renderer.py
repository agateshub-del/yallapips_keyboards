...
def render_long_display(profit: float = 0.0,
    ...
    """
    90x432 portrait image for the long strip display (device key 18).
    Device shows it as-is (no rotation applied for key 18).
    Three sections stacked vertically: SESSION / LOCAL TIME / OPEN P&L
    Session auto-detected from UTC — correct anywhere in the world.
    """
    W, H  = 90, 432
    ...
    SH  = 144      # each section height
    CX  = W // 2
    PAD = 6

    def _fit(text, max_size, bold=False):
        """Largest font that fits text within PAD margins."""
    ...
    # ── SECTION 1: SESSION (y=0..143) ─────────────────────────────
    draw.rectangle([(0, 0),   (W-1, SH-1)], fill=tuple(c//7 for c in sc))
    draw.rectangle([(0, 0),   (W-1, 4)],    fill=sc)
    draw.ellipse(  [(PAD, 10),(PAD+10, 20)], fill=sc)
    draw.text((CX, 36),  "SESSION",
              font=_font(10), fill=(90,100,120), anchor="mm")
    draw.text((CX, 76),  sn,
              font=_fit(sn, 18, bold=True), fill=sc, anchor="mm")
    draw.text((CX, 108), utc_now.strftime("%H:%M"),
              font=_font(22, bold=True), fill=sc, anchor="mm")
    draw.text((CX, 130), "UTC",
              font=_font(9), fill=(70,80,100), anchor="mm")
    draw.rectangle([(0, SH-1),(W-1, SH)], fill=(25,30,45))

    # ── SECTION 2: LOCAL TIME (y=145..288) ────────────────────────
    y2 = SH + 1
    draw.rectangle([(0, y2),(W-1, y2+SH-1)], fill=(10,12,20))
    draw.text((CX, y2+18),  "LOCAL TIME",
              font=_font(10), fill=(55,65,85), anchor="mm")
    draw.text((CX, y2+58),  local_now.strftime("%H:%M"),
              font=_font(26, bold=True), fill=(215,225,245), anchor="mm")
    draw.text((CX, y2+86),  local_now.strftime(":%S"),
              font=_font(20, bold=True), fill=(160,170,195), anchor="mm")
    draw.text((CX, y2+112), local_now.strftime("%d %b"),
              font=_font(14, bold=True), fill=(90,100,120), anchor="mm")
    draw.text((CX, y2+132), local_now.strftime("%Y"),
              font=_font(11), fill=(65,75,95), anchor="mm")
    draw.rectangle([(0, y2+SH-1),(W-1, y2+SH)], fill=(25,30,45))

    # ── SECTION 3: P&L (y=289..431) ──────────────────────────────
    y3 = y2 + SH + 1
    if connected:
        pc  = (0, 210, 100) if profit >= 0 else (240, 40, 60)
        bg3 = tuple(c//7 for c in pc)
        draw.rectangle([(0, y3),  (W-1, H-1)], fill=bg3)
        draw.rectangle([(0, H-5),(W-1, H-1)],  fill=pc)
        draw.text((CX, y3+18), "OPEN P&L",
                  font=_font(10), fill=(65,75,65), anchor="mm")
        draw.text((CX, y3+56), "▲" if profit >= 0 else "▼",
                  font=_font(30, bold=True), fill=pc, anchor="mm")
        amt_str = f"{profit:+.2f}"
        draw.text((CX, y3+98), amt_str,
                  font=_fit(amt_str, 22, bold=True), fill=pc, anchor="mm")
        draw.text((CX, y3+126), currency,
                  font=_font(12), fill=(65,75,65), anchor="mm")
    else:
        draw.rectangle([(0, y3),(W-1, H-1)], fill=(14,8,8))
        draw.rectangle([(0, H-5),(W-1,H-1)], fill=(80,30,30))
        draw.text((CX, y3+55), "MT5",     font=_font(22,bold=True), fill=(130,40,40), anchor="mm")
        draw.text((CX, y3+85), "OFFLINE", font=_font(14),           fill=(100,30,30), anchor="mm")

    draw.rectangle([(0,0),(W-1,H-1)], outline=(30,36,55), width=1)
    return img
