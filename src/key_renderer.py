...
def render_long_display(profit: float = 0.0,
    ...
    """
    432x90 landscape image for the long strip display (device key 18).
    Device rotates 90 CW internally → displays as 90x432 portrait strip.
    Three equal sections (left to right = top to bottom on device):
      SESSION | LOCAL TIME | OPEN P&L
    Session auto-detected from UTC (correct everywhere in the world).
    """
    W, H = 432, 90
    ...
    S = 144   # each section width
    sc = sess_color

    # ── SECTION 1: Session (x=0..143) ─────────────────────────────
    draw.rectangle([(0, 0),(S-1, H-1)], fill=tuple(c//7 for c in sc))
    draw.rectangle([(0, 0),(5,   H-1)], fill=sc)
    draw.ellipse(  [(12,8),(24, 20)],   fill=sc)
    draw.text((S//2+3, 15), "SESSION",
              font=_font(10), fill=(90,100,120), anchor="mm")
    fsz = 22 if len(sess_name) <= 8 else 16 if len(sess_name) <= 12 else 13
    draw.text((S//2+3, 50), sess_name,
              font=_font(fsz, bold=True), fill=sc, anchor="mm")
    draw.text((S//2+3, 76), utc_now.strftime("UTC  %H:%M"),
              font=_font(11), fill=(70,80,100), anchor="mm")
    draw.rectangle([(S-1,0),(S, H-1)], fill=(25,30,45))

    # ── SECTION 2: Local time (x=145..288) ────────────────────────
    x2 = S + 1
    draw.rectangle([(x2, 0),(x2+S-1, H-1)], fill=(10,12,20))
    draw.text((x2+S//2, 16), "◷  LOCAL  TIME",
              font=_font(10), fill=(55,65,85), anchor="mm")
    draw.text((x2+S//2, 50), local_now.strftime("%H:%M:%S"),
              font=_font(28, bold=True), fill=(215,225,245), anchor="mm")
    draw.text((x2+S//2, 76), local_now.strftime("%a  %d %b %Y"),
              font=_font(11), fill=(75,85,105), anchor="mm")
    draw.rectangle([(x2+S-1,0),(x2+S, H-1)], fill=(25,30,45))

    # ── SECTION 3: P&L (x=289..431) ──────────────────────────────
    x3 = x2 + S + 1
    if connected:
        pc  = (0, 210, 100) if profit >= 0 else (240, 40, 60)
        bg3 = tuple(c//7 for c in pc)
        draw.rectangle([(x3, 0),(W-1, H-1)], fill=bg3)
        draw.rectangle([(W-6, 0),(W-1, H-1)], fill=pc)
        draw.text((x3+S//2-3, 16), "OPEN  P&L",
                  font=_font(10), fill=(65,75,65), anchor="mm")
        arrow = "▲" if profit >= 0 else "▼"
        draw.text((x3+S//2-3, 50), f"{arrow}  {profit:+.2f}",
                  font=_font(24, bold=True), fill=pc, anchor="mm")
        draw.text((x3+S//2-3, 76), currency,
                  font=_font(11), fill=(65,75,65), anchor="mm")
    else:
        draw.rectangle([(x3, 0),(W-1, H-1)], fill=(14,8,8))
        draw.rectangle([(W-6, 0),(W-1, H-1)], fill=(80,30,30))
        draw.text((x3+S//2-3, 38), "MT5",
                  font=_font(22, bold=True), fill=(130,40,40), anchor="mm")
        draw.text((x3+S//2-3, 65), "OFFLINE",
                  font=_font(13), fill=(100,30,30), anchor="mm")

    draw.rectangle([(0,0),(W-1,H-1)], outline=(28,34,52), width=1)
    return img
