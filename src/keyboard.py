...
class YallaPipsKeyboard:
    ...
    def _render_splash(self):
        """Show YALLA PIPS branding on all keys during startup."""
        for i in range(self._dev.key_count):
            self._push(i, kr.render_splash(i + 1))
        import time; time.sleep(1.5)   # hold for 1.5s
    ...
    def _refresh_long_display(self):
        ...
        # Send 3 separate panels to keys 16, 17, 18
        panels = kr.render_long_display_panels(
            profit=profit, currency=currency, connected=connected)
        try:
