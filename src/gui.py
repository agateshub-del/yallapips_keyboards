...
def show_settings_window():
    ...
    # ── Broker Type ───────────────────────────────────────────────
    broker_frame = ttk.LabelFrame(root, text="  Broker Decimal Type  ")
    broker_frame.grid(row=base_row+1, column=0, columnspan=2,
                      padx=12, pady=(6,4), sticky="ew")

    broker_type = tk.StringVar(value=get("broker_type", "auto"))

    ttk.Radiobutton(broker_frame,
                    text="Auto-detect (recommended)",
                    variable=broker_type, value="auto"
                    ).grid(row=0, column=0, sticky="w", padx=10, pady=(6,2))
    ttk.Radiobutton(broker_frame,
                    text="2-digit  (e.g. XAUUSD = 3350.25)  ← Vantage, IC Markets",
                    variable=broker_type, value="2digit"
                    ).grid(row=1, column=0, sticky="w", padx=10, pady=2)
    ttk.Radiobutton(broker_frame,
                    text="3-digit  (e.g. XAUUSD = 3350.250)  ← Exness",
                    variable=broker_type, value="3digit"
                    ).grid(row=2, column=0, sticky="w", padx=10, pady=(2,4))
    tk.Label(broker_frame,
             text="SL/TP Points are always entered in PIPS — this adjusts internally per broker.",
             bg="#0a0c14", fg="#5070a0",
             font=("Arial",8,"italic")).grid(row=3, column=0, sticky="w",
                                              padx=14, pady=(0,6))

    # ── Close Mode ────────────────────────────────────────────────
    mode_frame = ttk.LabelFrame(root, text="  25 / 50 / 75 %  Close Mode  ")
    mode_frame.grid(row=base_row+2, column=0, columnspan=2,
    ...
    tk.Label(root, textvariable=status_var,
             bg="#0a0c14", fg="#00d264",
             font=("Arial",10,"bold")).grid(row=base_row+3, column=0,
                                             columnspan=2, pady=4)
    ...
    btn_frame.grid(row=base_row+4, column=0, columnspan=2, pady=(8,16))
