...
def render_long_display(profit: float = 0.0,
    ...
    """
    96x96 image for the long strip display (device key 18).
    The actual LCD on the strip is 96x96 — same as a regular key.
    Compact 3-section layout: SESSION / LOCAL TIME / P&L
    set_long_display applies FLIP_TOP_BOTTOM to match device orientation.
    """
    W, H = 96, 96
    ...
    # ── SESSION (y 0-29) ──────────────────────────────────────────
    draw.rectangle([(0, 0), (W-1, 29)], fill=tuple(c//6 for c in sc))
    draw.rectangle([(0, 0), (W-1, 3)],  fill=sc)
    draw.text((CX, 16), sn,
              font=fit(sn, 13, True), fill=sc, anchor="mm")
    draw.text((CX, 26), utc_now.strftime("UTC %H:%M"),
              font=_font(8), fill=tuple(c//2 for c in sc), anchor="mm")
    draw.rectangle([(0, 29),(W-1, 30)], fill=(25,30,45))

    # ── LOCAL TIME (y 31-61) ──────────────────────────────────────
    draw.rectangle([(0, 31),(W-1, 61)], fill=(10,12,20))
    draw.text((CX, 40), local_now.strftime("%H:%M"),
              font=_font(16, True), fill=(215,225,245), anchor="mm")
    draw.text((CX, 55), local_now.strftime(":%S  %d %b"),
              font=_font(9), fill=(120,130,150), anchor="mm")
    draw.rectangle([(0, 61),(W-1, 62)], fill=(25,30,45))

    # ── P&L (y 63-95) ─────────────────────────────────────────────
    if connected:
        pc = (0, 210, 100) if profit >= 0 else (240, 40, 60)
        draw.rectangle([(0, 63),(W-1, 95)], fill=tuple(c//7 for c in pc))
        draw.rectangle([(0, 92),(W-1, 95)], fill=pc)
        draw.text((CX, 74), ("▲" if profit >= 0 else "▼") + " OPEN P&L",
                  font=_font(9), fill=pc, anchor="mm")
        draw.text((CX, 87), f"{profit:+.2f}",
                  font=fit(f"{profit:+.2f}", 13, True), fill=pc, anchor="mm")
    else:
        draw.rectangle([(0, 63),(W-1, 95)], fill=(14,8,8))
        draw.text((CX, 82), "MT5 OFFLINE",
                  font=_font(9), fill=(130,40,40), anchor="mm")

    draw.rectangle([(0,0),(W-1,H-1)], outline=sc, width=1)
    return img
