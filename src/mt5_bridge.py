...
def _ensure_symbol(symbol: str) -> bool:
    """Make sure symbol is in Market Watch. Auto-tries common suffixes."""
    ...
    # Symbol not found — try common broker suffixes automatically
    suffixes = ["m", ".", ".a", "+", "#", "_", "c", "pro", "raw"]
    for sfx in suffixes:
        candidate = symbol + sfx
        si2 = mt5.symbol_info(candidate)
        if si2 is not None:
            Logger.warning(
                f"Symbol '{symbol}' not found — auto-switching to '{candidate}'. "
                f"Update Symbol in Settings to stop this warning."
            )
            if not si2.visible:
                mt5.symbol_select(candidate, True)
            # Patch config so trades use correct symbol this session
            try:
                from src.config import set_and_save
                set_and_save({"symbol": candidate})
                Logger.info(f"Symbol auto-corrected to '{candidate}' in config")
            except Exception:
                pass
            return True

    # Last attempt: search all symbols for partial match
    all_syms = mt5.symbols_get()
    if all_syms:
        base = symbol.upper()
        matches = [s.name for s in all_syms
                   if base in s.name.upper() and s.name != symbol]
        if matches:
            Logger.error(
                f"Symbol '{symbol}' not found. Did you mean: {', '.join(matches[:5])}? "
                f"Update Symbol in Settings."
            )
