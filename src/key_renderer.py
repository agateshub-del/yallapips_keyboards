...
def render_long_display(profit: float = 0.0,
    ...
    """
    90x300 portrait image for the long strip display (device key 18).
    Device flips vertically — pre-flipped in set_long_display to compensate.
    Three sections: SESSION (top) / LOCAL TIME (mid) / OPEN P&L (bottom)
    """
    W, H = 90, 300
    ...
    draw.ellipse(  [(PAD, 8), (PAD+9, 17)], fill=sc)
    draw.text((CX, 26), "SESSION",
              font=_font(9), fill=(90,100,120), anchor="mm")
    draw.text((CX, 57), sn,
              font=_fit(sn, 17, True), fill=sc, anchor="mm")
    draw.text((CX, 80), utc_now.strftime("%H:%M"),
              font=_font(16, True), fill=sc, anchor="mm")
    draw.text((CX, 93), "UTC",
              font=_font(8), fill=(70,80,100), anchor="mm")
    draw.rectangle([(0, SH-1),(W-1, SH)], fill=(25,30,45))

    # ── LOCAL TIME (y=101..200) ───────────────────────────────────
    y2 = SH + 1
    draw.rectangle([(0, y2),(W-1, y2+SH-1)], fill=(10,12,20))
    draw.text((CX, y2+13), "LOCAL TIME",
              font=_font(9), fill=(55,65,85), anchor="mm")
    draw.text((CX, y2+42), local_now.strftime("%H:%M"),
              font=_font(22, True), fill=(215,225,245), anchor="mm")
    draw.text((CX, y2+63), local_now.strftime(":%S"),
              font=_font(16, True), fill=(160,170,195), anchor="mm")
    draw.text((CX, y2+81), local_now.strftime("%d %b"),
              font=_font(12, True), fill=(90,100,120), anchor="mm")
    draw.text((CX, y2+95), local_now.strftime("%Y"),
              font=_font(9), fill=(65,75,95), anchor="mm")
    ...
    if connected:
        ...
        draw.text((CX, y3+13), "OPEN P&L",
                  font=_font(9), fill=(65,75,65), anchor="mm")
        draw.text((CX, y3+42), "▲" if profit >= 0 else "▼",
                  font=_font(24, True), fill=pc, anchor="mm")
        draw.text((CX, y3+72), f"{profit:+.2f}",
                  font=_fit(f"{profit:+.2f}", 18, True), fill=pc, anchor="mm")
        draw.text((CX, y3+89), currency,
                  font=_font(10), fill=(65,75,65), anchor="mm")
    else:
        draw.rectangle([(0, y3),(W-1, H-1)], fill=(14,8,8))
        draw.rectangle([(0, H-5),(W-1,H-1)], fill=(80,30,30))
        draw.text((CX, y3+40), "MT5",     font=_font(20,True), fill=(130,40,40), anchor="mm")
        draw.text((CX, y3+65), "OFFLINE", font=_font(12),      fill=(100,30,30), anchor="mm")
