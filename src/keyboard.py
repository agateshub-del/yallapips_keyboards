...
Central controller: maps 15 keys to actions, manages display updates.
"""
import threading
import time
import logging
...
# ── Key index → display update function (idle state) ──────────────
KEY_COUNT = 15
...
class YallaPipsKeyboard:
    def __init__(self, device: hardware.StreamDockDevice):
        ...
        self._mt5_timer  = None
        self._be_timer   = None
        self._trail_timer = None
    ...
    # ── Render all keys in their idle state ───────────────────────
    ...
    def _flash(self, idx: int, ok: bool, msg: str = ""):
    ...
    def _buy(self):
        r = mt5b.buy(get("symbol"), get("lots"), get("sl_points"), get("tp_points"), get("magic"))
        self._flash(0, r.get("success", False), f"{r.get('price',0):.2f}" if r.get("success") else "FAIL")

    def _sell(self):
        r = mt5b.sell(get("symbol"), get("lots"), get("sl_points"), get("tp_points"), get("magic"))
        self._flash(1, r.get("success", False), f"{r.get('price',0):.2f}" if r.get("success") else "FAIL")
    ...
    def _start_mt5_ticker(self):
        def tick():
            while True:
                ...
                try:
                    self._refresh_mt5_key()
                    # Also refresh buy/sell keys with latest price
                    price = self._price()
                    self._push(0, kr.render_buy(price))
                    self._push(1, kr.render_sell(price))
                except Exception:
    ...
    def _start_trail_monitor(self):
        def tick():
            while self._trailing:
                try:
                    mt5b.update_trailing(get("trail_pips", 15), get("trail_step", 5), self._sym())
                except Exception:
                    pass
                time.sleep(1)
        t = threading.Thread(target=tick, daemon=True)
