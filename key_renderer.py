...
def render_long_display(profit: float = 0.0,
    ...
    """
    96×96 image for the right-side long strip display.
    Shows: session, local time, P&L.
    Session is determined from local system time (auto-detects timezone).
    """
    ...
    # Show top session (most relevant)
    sess_name, sess_color = sessions[-1]
    # If overlap, show both names
    if len(sessions) > 1:
        sess_name = "/".join(s[0][:3] for s in sessions)

    # ── Dividers ──────────────────────────────────────────────────
    draw.rectangle([(1,1),(94,94)], outline=sess_color, width=2)

    # ── Section 1: Session ────────────────────────────────────────
    # Background strip
    draw.rectangle([(2, 2),(93, 30)], fill=tuple(c//4 for c in sess_color))
    draw.text((48, 16), sess_name, font=_font(11, bold=True),
              fill=sess_color, anchor="mm")

    # Session dot indicator
    draw.ellipse([(6,8),(16,18)], fill=sess_color)

    draw.line([(2,31),(93,31)], fill=(30,35,50), width=1)

    # ── Section 2: Local time ─────────────────────────────────────
    time_str = local_now.strftime("%H:%M:%S")
    date_str = local_now.strftime("%d %b")
    draw.text((48, 48), time_str, font=_font(14, bold=True),
              fill=(220, 220, 220), anchor="mm")
    draw.text((48, 62), date_str, font=_font(9),
              fill=(120, 120, 140), anchor="mm")

    draw.line([(2,67),(93,67)], fill=(30,35,50), width=1)

    # ── Section 3: P&L ───────────────────────────────────────────
    if connected:
        pnl_color = C_GREEN if profit >= 0 else C_RED
        pnl_str   = f"{profit:+.2f}"
        draw.text((48, 78), "P&L",   font=_font(9),             fill=(120,120,140), anchor="mm")
        draw.text((48, 90), pnl_str, font=_font(11, bold=True), fill=pnl_color,    anchor="mm")
    else:
        draw.text((48, 82), "MT5 OFF", font=_font(10), fill=(80,80,90), anchor="mm")
